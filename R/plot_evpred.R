...
#' The plot produced depends on \code{object$type}.
#' If \code{object$type = "d", "p"} or \code{"q"} then
...
#' with a line for each value of N in \code{object$n_years}.
#' If \code{object$type = "r"} then estimates of the predictive density
#' (from \code{\link[stats]{density}}) are plotted with a line for each N.
#' If \code{object$type = "i"} then lines representing estimated predictive
...
#'   See \code{\link{legend}}.
#' @param which_int.  A character scalar.  If \code{object$type = "i"} which
...
plot.evpred <- function(object, ...,
  ...
  if (object$type == "i") {
    ...
    if (is.null(temp$ylab)) {
      title(ylab = "time horizon N, in years")
    }
    my_title(...)
  }
  if (object$type == "r") {
  ...
  if (object$type %in% c("p", "d", "q", "r")) {
    ...
    if (is.null(temp$xlab)) {
      title(xlab = switch(type, p = "quantile", d = "quantile",
                          r = "quantile", q = "probability", "i" = "quantile"))
    }
    if (is.null(temp$ylab)) {
      title(ylab = switch(type, p = "probability", d = "density",
