...
The **revdbayes** packages provides functions for performing maximum likelihood and Bayesian inferences about $\theta$ under the K-gaps model.  We illustrate the code using the `newlyn` dataset, which is analysed in @FW2012.
...
mle_ci <- kgaps_mle(newlyn, thresh, conf = 95)
mle_ci$theta_mle
mle_ci$theta_se
mle_ci$theta_ci
```
...
The function `kgaps_post` simulates a random sample from the posterior distribution of $\theta$ based on a Beta($\alpha, \beta$) prior.  The use can choose the values of $\alpha$ and $\beta$.  The default is that $\alpha = \beta = 1$, that is, a U(0,1) prior for $\theta$.  See @Attalides2015 for further information and for a methods for selecting the value of the threshold in this situation.

```{r, fig.width = 5, fig.align='center'}
thresh <- quantile(newlyn, probs = 0.90)
k_postsim <- kgaps_post(newlyn, thresh)
plot(k_postsim, xlab = expression(theta))
```
