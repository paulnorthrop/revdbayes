...
test_that("pgev, values and length of output", {
  expect_equal(pgev(xvec, shape = xi1), c(0, exp(-1), 1, NA))
  ...
  expect_equal(pgev(xvec, shape = xi2), c(0, exp(-1), 1, NA))
  ...
  expect_equal(pgev(xvec, shape = xi3), c(0, exp(-1), 1, NA))
...
test_that("qgev, values and length of output", {
  ...
  expect_equal(qgev(pvec, shape = xi1), c(-10, q1, Inf, NA))
  ...
  expect_equal(qgev(pvec, shape = xi3), c(-Inf, q3, 10, NA))
